...
# 🔹 Word2Vec 벡터 데이터 로드
word2vec_url = "https://raw.githubusercontent.com/ji2won/khuda-team3/main/word2vec_vectors.xlsx"  # 실제 경로 입력
...
try:
    ...
    # 🔹 2. 엑셀 파일을 BytesIO로 변환 후 pandas로 읽기
    word_vectors_df = pd.read_excel(io.BytesIO(response.content), index_col=0)

    # 🔹 3. 데이터 확인
    if word_vectors_df.shape[0] == 0:
        raise ValueError("❌ Word2Vec 데이터가 비어 있습니다. 파일을 확인하세요.")

    print("✅ Word2Vec 데이터 로드 성공!")
    print("데이터 크기:", word_vectors_df.shape)  # 데이터 크기 확인
...
# 🔹 [2] 유사한 메뉴 추천 함수 (Word2Vec 기반 + 가게 연결)
def recommend_similar_menus(input_word, top_n=20, similarity_threshold=0.5):
    ...
    # 2️⃣ 20개 미만이면, 추가적으로 연관된 메뉴 검색 (어절 확장)
    ...
    # 3️⃣ 20개 미만이면, Word2Vec 기반 유사 메뉴 추가 추천
    if len(matched_menus_with_stores) < top_n:
        try:
